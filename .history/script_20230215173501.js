...
// // SYNTAX       Array.from(arrayLike[, mapFn[, thisArg]])    .arrayLike: Массивоподобный или итерируемый объект, преобразуемый в массив.  mapFn: Необязательный Отображающая функция, вызываемая для каждого элемента массива.  thisArg: Необязательный Значение, используемое в качестве this при выполнении функции mapFn.
// const y = Array.from({ length: 7 }, () => 1);
// console.log(y); // [1, 1, 1, 1, 1, 1, 1]

// const z = Array.from({ length: 7 }, (_, i) => i + 1); // _ не используем первый параметр
// console.log(z); // [1, 2, 3, 4, 5, 6, 7]

// // Массив из произвольных чисел , длиной 100
...
//     el => Number(el.textContent.replace('€', '')) // удаляем значки евро и формируем
//   );
...
const numDeposits1000 = accounts
  .flatMap(acc => acc.movements)
  .reduce((count, cur) => (cur >= 1000 ? ++count : count), 0); // 6 // Если сумма больше, чем 1000? то возвращаем счётчик +1 : иначе оставляем счётчик без изменений
console.log(numDeposits1000);

// 3.  заполняем с пом. Reduce новый объект, который выступает в роли стартового значение метода Reduce. { deposits: 0, withdrawals: 0 }
const sums = accounts // можно сразу деструктурировать  const {deposits, withdrawals} = ....
  .flatMap(acc => acc.movements)
  .reduce(
    (sums, cur) => {
      ...
      return sums; // должны возвращать Аккумулятор, если есть фигурные скобки
    },
    { deposits: 0, withdrawals: 0 }
  ); //начальная позиция в reduce - новый объект с какими-то значениями
console.log(sums);
...
const convertTitleCase = function (title) {
  ...
  const titleCase = title
    ...
    .split(' ')
    .map(
      word => (exceptions.includes(word) ? word : capitalize(word)) // если слово есть в списке исключений, то вернуть слово без изменений
    )
    .join(' '); // объединить всё предложение
...
const ownersEatTolittle = dogs
  .filter(dog => dog.curFood < dog.recFood)
  .flatMap(dog => dog.owners);
console.log(ownersEatTolittle); // ['Alice', 'Bob', 'Michael']

// 4.
// ownersEatToomuch.forEach(owner => console.log(owner));
...
// 6.
const checkEaringOkay = dog =>
  dog.curFood > dog.recFood * 0.9 && dog.recFood < dog.recFood * 1.1;

console.log(dogs.some()); // true
...
// const overalBalance2 = accounts
// .flatMap(acc => acc.movements)
// const account = dogs.find(dog => dog.owners.flat(1) == 'Sarah');
// console.log(account);
