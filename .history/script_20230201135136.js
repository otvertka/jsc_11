...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  const juliaCorrecteded = juliaCopy.splice(1, `${juliaCopy.length}` - 2, -1); // удаляем Первый и Последний элем (средний параметр это длина всей строки - 2 элеммента)

  const allDogs = juliaCorrecteded.concat(dogsKate);
  console.log(allDogs);
  // return function
  // console.log(juliaCorrecteded);
};
